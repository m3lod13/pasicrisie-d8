...
namespace Drupal\migrate_use*\Plugin\migrate\source;
...
class User extends DrupalSqlBase implements SqlBase {
  ...
  public function fields() {
    ...
    $fields['date_expire'] = $this->t('Date expire');
    return $fields;
  }
  ...
  public function prepareRow(Row $row) {
    ...
    $result = $this->getDatabase()->query('
      SELECT
        fld.field_date_expire,
      FROM
        {field_data_field_date_expire} fld
      WHERE
        fld.entity_id = :uid
    ', array(':uid' => $uid));
    foreach ($result as $record) {
      $row->setSourceProperty('date_expire', $record->field_date_expire_value );
